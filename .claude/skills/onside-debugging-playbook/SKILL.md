...
| Open-now chip filters nothing out, or seems wrong for a specific venue | None needed: this is a KNOWN limitation, not a regression. `ExploreScreen.tsx` calls `isOpenNow(undefined, now)` because the search projection does not include `venues.hours` (`SearchResult.venue` in `src/types/api.ts` picks only id/name/lat/lng/address/photos/venue_type). | Every venue is evaluated against the default 06:00 to 23:00 fallback window, identically. The venue DETAIL screen is exact (it has `hours` and uses `openStatus`, which hides the row when data is missing rather than guessing). | Open issue #475 (add hours to the search RPC projection + API mapping + app type). Do not "fix" by asserting hours client-side. |
...
- Open-now default window + missing hours in search projection: `grep -n "isOpenNow(undefined" fieldstack-app/src/screens/main/ExploreScreen.tsx` and `gh issue view 475 --json state`
