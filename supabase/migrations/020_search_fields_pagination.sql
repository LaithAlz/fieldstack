-- Add p_limit / p_offset to search_fields so the API can serve pages.
-- The total count is computed from the un-paginated matched CTE so callers
-- know how many pages exist without a separate count query.
-- Drops the prior 8-arg signature; PostgREST routes by arg name + count.
...
grant execute on function search_fields(
  float8, float8, float8, field_surface[], field_size[], venue_type[], numeric, text, int, int
